...
The instinct is to teach it. Fine-tune on your codebase, update the weights, make the model _know_ your architecture as it knows Python's standard library. No retrieval latency, no context-window juggling. Just a model that understands your system.
...
1. **Retrieval-augmented generation.** Index the code, retrieve context at query time, feed it into the prompt. The model doesn't need to _know_ your codebase. It needs to _see_ the relevant parts.
...
The tradeoff is retrieval precision. A fine-tuned model would _just know_ things. A retrieval-augmented model needs the right context to surface at the right time. That's a search problem, not a training problem. Search problems yield to deterministic tools.
